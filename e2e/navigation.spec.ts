...
test.describe('Header', () => {
  ...
  test('shows the active conference name', async ({ page }) => {
    ...
    const conferenceButton = page.getByRole('button', { name: /sea|hamvention|pacificon|huntsville|hamcation|yuma/i }).first();
  ...
  test('conference selector dialog opens and lists conferences', async ({ page }) => {
    ...
    const selectorButton = page.locator('button:has(svg)').filter({ hasText: /sea|hamvention|pacificon|huntsville|hamcation|yuma/i }).first();
    await selectorButton.click();
    // Dialog should appear
    await expect(page.getByRole('dialog')).toBeVisible();
    await expect(page.getByText('Select Conference')).toBeVisible();
    // At least one conference card should be listed
    await expect(page.locator('[role="dialog"] [class*="cursor-pointer"]').first()).toBeVisible();
  });
  ...
  test('header collapse button toggles header', async ({ page }) => {
    ...
    const venueDetail = page.locator('svg').filter({ has: page.locator('~ span:text("map")') });
