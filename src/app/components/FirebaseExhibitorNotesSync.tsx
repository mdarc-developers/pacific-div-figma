 ...
 * - On user login (or conference change while logged in): loads saved exhibitor notes
 *   from Firestore and applies them via the shared ExhibitorNotesContext.
 ...
 */
export function FirebaseExhibitorNotesSync() {
  const { user } = useAuth();
  const { activeConference } = useConference();
  const { notes, overrideNotes } = useExhibitorNotesContext();
  ...
  useEffect(() => {
    ...
    getUserExhibitorNotes(uidToLoad, conferenceId)
      .then((savedNotes) => {
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    setUserExhibitorNotes(user.uid, conferenceId, notes).catch(console.error);
