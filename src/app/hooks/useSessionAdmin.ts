 ...
 * "session-admin" group, determined by checking:
 *
 * 1. `mapUserProfileGroups` entries (matched by uid).
 * 2. `mapUserProfiles` entries whose `groups` array includes "session-admin"
 *    (matched by email).
 *
 ...
 * calling components.
 */
export function useSessionAdmin(): boolean {
  const { user } = useAuth();
  ...
  return profile?.groups?.includes("session-admin") ?? false;
}
